...
if(isset($_SESSION['log_activity_filter'])){
	$query="SELECT 'Activity', log_id AS id, log_date AS date, field_name AS fname, field_replication_number AS frn, plots AS plot, activity_name AS item, log.user_id AS user FROM field, activity, log WHERE field.field_id = log.field_id".$_SESSION['log_field_filter'].$_SESSION['log_date_filter'].$_SESSION['log_user_filter']."AND activity.activity_id = log.activity_id AND log.activity_id=".$_SESSION['log_activity_filter']." ORDER BY date DESC, item, fname, frn, plot LIMIT $from, $max_messages";
} else if(isset($_SESSION['log_measurement_filter'])){
	$query="SELECT 'Measurement', log_id AS id, log_date AS date, field_name AS fname, field_replication_number AS frn, plots AS plot, measurement_name AS item, log.user_id AS user FROM field, measurement, log WHERE field.field_id = log.field_id".$_SESSION['log_field_filter'].$_SESSION['log_date_filter'].$_SESSION['log_user_filter']."AND measurement.measurement_id = log.measurement_id AND log.measurement_id=".$_SESSION['log_measurement_filter']." ORDER BY date DESC, item, fname, frn, plot LIMIT $from, $max_messages";
} else {
	$query="(SELECT 'Activity' AS item_type, log_id AS id, log_date AS date, field_name AS fname, field_replication_number AS frn, plots AS plot, activity_name AS item, log.user_id AS user FROM field, activity, log WHERE field.field_id = log.field_id".$_SESSION['log_field_filter'].$_SESSION['log_date_filter'].$_SESSION['log_user_filter']."AND activity.activity_id = log.activity_id AND log.activity_id>0) UNION (SELECT 'Measurement' AS item_type, log_id AS id, log_date AS date, field_name AS fname, field_replication_number AS frn, plots AS plot, measurement_name AS item, log.user_id AS user FROM field, measurement, log WHERE field.field_id = log.field_id".$_SESSION['log_field_filter'].$_SESSION['log_date_filter'].$_SESSION['log_user_filter']."AND measurement.measurement_id = log.measurement_id AND log.measurement_id>0) UNION (SELECT 'Input' AS item_type, input_log_id AS id, input_log_date AS date, field_name AS fname, field_replication_number AS frn, plots AS plot, crop_name AS item, input_log.user_id AS user FROM field, crop, input_log WHERE field.field_id = input_log.field_id".$_SESSION['input_log_field_filter'].$_SESSION['input_log_date_filter'].$_SESSION['input_log_user_filter']."AND crop.crop_id = input_log.crop_id AND input_log.crop_id>0) UNION (SELECT 'Input' AS item_type, input_log_id AS id, input_log_date AS date, field_name AS fname, field_replication_number AS frn, plots AS plot, treatment_name AS item, input_log.user_id AS user FROM field, treatment, input_log WHERE field.field_id = input_log.field_id".$_SESSION['input_log_field_filter'].$_SESSION['input_log_date_filter'].$_SESSION['input_log_user_filter']."AND treatment.treatment_id = input_log.treatment_id AND input_log.treatment_id>0) ORDER BY date DESC, item_type, item, fname, frn, plot LIMIT $from, $max_messages";
}
$result = mysqli_query($dbh,$query);
$n=0;
$cb=0;
